...
def run_remote_script(event, context):
    ...
    remote script resides in github. This was originally developed to run a
    database backup script on a local mariadb/mysql server on the Coral server.
    It was adapted from proxy's remote config update function.
    ...
    dumpBucket, dumpPath, dbList, dbHost, dumpUserPath, dumpUserPwPath =>
        Who dumps, where they dump from, what they dump, and where it ends up.
    """
    print("### Starting")
    print("### Event:")
    print("{}".format(event))
    ...
    dump_bucket = os.environ.get('dumpBucket', default='ec2.lib.fsu.edu')
    dump_path = os.environ.get('dumpPath', default='coral-systems/db/backups')
    db_list = os.environ.get('dbList', default='all')
    dump_user_path = os.environ.get('dumpUserPath', default=None)
    dump_user_pw_path = os.environ.get('dumpUserPwPath', default=None)
    db_host = os.environ.get('dbHost', default='localhost')
    ...
    try:
        ...
        if dump_user_path is None:
            raise RunRemoteScriptException("No SSM path to a db user provided.")
        if dump_user_pw_path is None:
            dump_user_pw_path = ''
        ...
        # Dr. Dumpenstein: bucket/path app user pw
        bucket_path = "/".join([dump_bucket, dump_path])
